...
def parse_options():
    ...
    parser.add_argument('-i', '--input', help='The dir path of input', type=str, default='/opt/data/VCM/data/bin/qemu/')
    parser.add_argument('-o', '--output', help='The dir path of output', type=str, default='/opt/data/VCM/data/pdg/qemu')
...
def joern_parse(file, outdir):
    ...
    os.system('sh /opt/data/VCM/joern/joern-cli/joern-parse $file --language c --output $out')
    #out.append(out)
...
def joern_export(bin, outdir, repr):
    ...
    if repr == 'pdg':
        os.system('sh /opt/data/VCM/joern/joern-cli/joern-export $bin'+ " --repr " + "pdg" + ' --out $out')
        try:
        ...
        if out[-4:] != 'json':
            out += '.json'
        joern_process = subprocess.Popen(["/opt/data/VCM/joern"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True, encoding='utf-8')
        import_cpg_cmd = f"importCpg(\"{bin}\")\r"
        script_path = f"/opt/data/VCM/joern/graph-for-funcs.sc"
...
def main():
    joern_path = '/opt/data/VCM/joern/joern-cli'
    os.chdir(joern_path)
    args = parse_options()
    type = args.type
    repr = args.repr
